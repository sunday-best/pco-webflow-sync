...
async function fetchAllPcoEvents(pcoToken, updatedSince = null) {
  ...
  while (nextUrl) {
    ...
    for (const signup of signups) {
      ...
      // Skip archived or link-only events (registration_state === 'link' means external link redirect)
      ...
      allEvents.push({
        ...
        'event.registration_open': attrs.registration_state === 'open',
        'event.registration_url': attrs.registration_url || '',
        'event.event_url': attrs.registration_url || '',
        ...
        'event.image_url': attrs.logo_url || attrs.featured_image_url || '',
        'event.thumbnail_url': attrs.logo_url || attrs.featured_image_url || '',
        'event.capacity': null,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Handle removed events - only when we fetched all PCO events (no updatedSince filter)
